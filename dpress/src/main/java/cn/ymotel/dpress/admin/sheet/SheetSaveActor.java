...
@ActorCfg(urlPatterns = "/api/admin/sheets/{id}",methods = RequestMethod.PUT)
public class SheetSaveActor implements Actor<ServletMessage> {
    @Autowired
    ...
    public Map Execute(ServletMessage message) throws Throwable {
        ...
        if(post.get("editorType")==null){
            ...
            post.put("editor_type", PostEditorType.valueOf((String)post.get("editorType")).getValue());
        ...
        if(post.get("createTime")==null){

        }else{
            post.put("create_time",new java.sql.Timestamp((Long)post.get("createTime")));
        }
        post.put("type",1);
        ...
        if(post.get("disallowComment")==null){
            post.put("disallow_comment",Boolean.FALSE);
        }else{
        ...
        if(post.get("id")==null||post.get("id").toString().trim().equals("")){
            ...
            post.put("update_time",new java.sql.Timestamp(System.currentTimeMillis()));
            post.put("edit_time",new java.sql.Timestamp(System.currentTimeMillis()));
            sqlSession.update("posts.u2",post);
            //update
        }
