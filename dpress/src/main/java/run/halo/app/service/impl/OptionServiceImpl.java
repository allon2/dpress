...
public class OptionServiceImpl extends AbstractCrudService<Option, Integer> implements OptionService {
    ...
    public void save(Map<String, Object> optionMap) {
        ...
        long siteid=Utils.getSiteIdFromMvc();
//        Map  optionKeyMap=sqlSession.selectMap("options.qall",pMap,"option_key");
        for(java.util.Iterator iter=optionMap.entrySet().iterator();iter.hasNext();){
            Map.Entry entry=(Map.Entry)iter.next();
            Map tMap=new HashMap();
            tMap.put("option_key",entry.getKey());
            tMap.put("option_value",entry.getValue().toString());
            tMap.put("siteid",siteid);
            tMap.put("update_time",new java.sql.Timestamp(System.currentTimeMillis()));
           int i= sqlSession.update("options.uoption",tMap);
           if(i==0){
               tMap.put("create_time",new java.sql.Timestamp(System.currentTimeMillis()));
               tMap.put("type",0);
               sqlSession.insert("options.ioption",tMap);
           }
        }
