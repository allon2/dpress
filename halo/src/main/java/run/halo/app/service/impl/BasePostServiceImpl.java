...
public abstract class BasePostServiceImpl<POST extends BasePost> extends AbstractCrudService<POST, Integer> implements BasePostService<POST> {
    ...
    public POST getBySlug(String slug) {
        ...
        return basePostRepository.getBySlugAndSiteid(slug,Utils.getSiteId()).orElseThrow(() -> new NotFoundException("查询不到该文章的信息").setErrorData(slug));
    }
    ...
    public Page<POST> pageBy(PostStatus status, Pageable pageable) {
        Assert.notNull(status, "Post status must not be null");
        Assert.notNull(pageable, "Page info must not be null");
        return basePostRepository.findAllByStatusAndSiteid(status, Utils.getSiteId(), pageable);
    }
